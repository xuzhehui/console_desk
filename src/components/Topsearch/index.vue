    ...
    <div style="padding:10px 0;">
        <div class="items" v-for="(item,index) in list" :key="index">
            ...
            <div class="items" v-if="item.isDate">
                <DatePicker :type='item.type ? item.type : "date"' v-model="item.start_value"  size='small'  :placeholder="item.start_placeholder" class="margin"></DatePicker>
                ~
                <DatePicker :type="item.type ?item.type : 'date'" v-model="item.end_value" size='small' :placeholder="item.end_placeholder" class="margin"></DatePicker>
            </div>
        </div>
